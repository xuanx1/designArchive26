...

{
  ...
  FLIPBOOK.PageWebGL = function (
    ...
    var mats2 = [basicMat, basicMat, basicMat, basicMat, basicMat, basicMat];
    ...
    if (this.pH > this.pW) this.bendF.switchAxes = true;

    this.b2 = new MOD3.Bend(0, 0, 0);
    this.b2.constraint = MOD3.ModConstant.LEFT;
    if (this.pH > this.pW) this.b2.switchAxes = true;
    this.b2.offset = 0.98;
    this.b2.setAngle(1);
  ...
  FLIPBOOK.PageWebGL.prototype.onPageCanvasLoaded = function (
    page,
    side,
    callback,
  ) {
  ...
  FLIPBOOK.PageWebGL.prototype.load = function (side, callback) {
    ...
    for (var i = 0; i < pages.length; i++) {
      var p = pages[i];
      if (p.flippingLeft || p.flippingRight) return;
    }

    this.disposed = false;

    var index = self.options.rightToLeft
      ? this.book.pages.length - this.index - 1
      : this.index;
    ...
    var size =
      pageSize < o.pageTextureSizeTreshold
  ...
  FLIPBOOK.PageWebGL.prototype._setAngle = function (angle, direction) {
    ...
    if (angle <= 180 && angle >= -180) {
      ...
      if (this.singlePage) {
        ...
        if (angle < -180) angle = -180;
        if (this.index == this.book.pages.length - 1) return;
      } else {
      ...
      if (this.isFlippedLeft)
        this.bendF.force =
          /*this.bendB.force =*/ (1.35 *
            Math.pow(-Math.abs(Math.cos(-angle / 2)), 1)) /
          Math.pow(this.pageHardness, 1.5);
      else
        this.bendF.force =
          /*this.bendB.force =*/ (1.35 *
            Math.pow(Math.abs(Math.sin(-angle / 2)), 1)) /
          Math.pow(this.pageHardness, 1.5);
      this.updateBend();
  ...
  FLIPBOOK.PageWebGL.prototype.flipLeft = function (onComplete) {
    ...
    if (
      ...
      if (this.duration > 0) {
        ...
        this.force = 0;
        var newForce = (1 + Math.random() * 0.5) / this.pageHardness;
        var newOffset = 0.1 + Math.random() * 0.2;
        this.to = {
  ...
  FLIPBOOK.PageWebGL.prototype.correctZOrder = function () {
    ...
    for (i = this.index - 1; i >= 0; i--) {
      this.book.pages[i].position.z =
        this.book.pages[i + 1].position.z - th / 2 - th / 2 - 1;
      ...
      if (this.index < this.book.pages.length - 1) {
        ...
        if (this.index > 0)
          this.book.pages[this.index].position.z =
            this.book.pages[this.index - 1].position.z + th / 2 + th / 2 - 1;
      }
  ...
  FLIPBOOK.PageWebGL.prototype.setFrontMat = function (m) {
    if (this.cube.material[4] === m) return;
  ...
  FLIPBOOK.PageWebGL.prototype.setBackMat = function (m) {
    if (this.cube.material[5] === m) return;
    ...
    if (self.flippingLeft) {
      if (self.index < self.book.pages.length - 1) {
        ...
        for (var i = self.index - 1; i >= 0; i--) {
          self.book.pages[i].position.z =
            self.book.pages[i + 1].position.z - th / 2 - th / 2 - 1;
        }
      } else {
        if (self.book.pages.length > 1)
          self.book.pages[self.index].position.z =
            self.book.pages[self.index - 1].position.z + th / 2 + th / 2 - 1;
      }
    }
...

{
  ...
  FLIPBOOK.BookWebGL.prototype.init3d = function () {
    ...
    if (this.options.hasHtmlContent) this.initHtmlContent();
    ...
    if (this.options.lights) {
      ...
      if (this.options.shadows) {
        ...
        var bg = new THREE.Mesh(
          new THREE.BoxGeometry(10000, 10000, 1, 1, 1, 1),
          mat,
        );
...
*/
    ...
    this.centerContainer.position.x =
      -this.pageW * 0.5 * this.centerContainer.scale.x;
    ...
    (function a() {
      if (self.rendering) {
        ...
        if (self.needsUpdate) {
          ...
          //self.lh.update()
          if (self.htmlLayer) self.cssRenderer.render(self.Scene, self.Camera);
        }
  ...
  FLIPBOOK.BookWebGL.prototype.initHtmlContent = function () {
    ...
    this.$pageC = jQuery(document.createElement("div"))
      ...
      .css({
        width:
          (2 * 1000 * this.options.pageWidth) / this.options.pageHeight + "px",
        height: "1000px",
        position: "absolute",
        top: "-500px",
        left: (-1000 * this.options.pageWidth) / this.options.pageHeight + "px",
  ...
  FLIPBOOK.BookWebGL.prototype.onSwipe = function (
    event,
    phase,
    direction,
    distance,
    ...
    if ((phase == "cancel" || phase == "end") && fingerCount <= 1) {
      ...
      if (distance < 0 && (!right || !right.dragging)) this.prevPage();
      else if (distance > 0 && (!left || !left.dragging)) this.nextPage();
      else if (distance == 0)
        this.clickedPage
      ...
      if (right) right.dragging = false;
      if (left) left.dragging = false;
    }
    ...
    if (phase == "move" && fingerCount <= 1) {
      ...
      if ((left && left.flipping) || (right && right.flipping)) return;
      ...
      //one is dragging
        ...
        if ((left && !right) || (left && !right.dragging)) {
          if (distance <= 0) left._setAngle(distance, direction);
  ...
  FLIPBOOK.BookWebGL.prototype.onResize = function () {
    ...
    if (o.responsiveView && w <= o.responsiveViewTreshold && r1 < 2 * r2) {
      ...
      if (r2 > r1) this.sc = (zoomMin * r1) / (r2 * s);
      else this.sc = 1;
      ...
      if (r1 < 2 * r2) this.sc = (zoomMin * r1) / (2 * r2 * s);
      else this.sc = 1;

      if (this.flippedleft == 0) this.focusRight();
      else if (this.flippedright == 0) this.focusLeft();
      else this.focusBoth();
    }
  ...
  FLIPBOOK.BookWebGL.prototype.createPages = function () {
    ...
    for (i = 0; i < p.length / 2; i++) {
      hardness =
        i == 0 || i == p.length / 2 - 1
          ? self.options.coverHardness
          : self.options.pageHardness;
      page = new FLIPBOOK.PageWebGL(
        self,
        i,
        hardness,
        self.options,
  ...
  FLIPBOOK.BookWebGL.prototype.goToPage = function (index, instant) {
    ...
    //if(index % 2 != 0) index++;
    if (index < 0) index = 0;
    if (index > this.options.pages.length) index = this.options.pages.length;
    if (index % 2 != 0) index--;
    ...
    if (typeof instant != "undefined" && instant) {
      ...
      if (index > self.rightIndex) {
        while (self.rightIndex < index) this.nextPageInstant();
      } else {
        while (self.rightIndex > index) this.prevPageInstant();
      }
    ...
    if (this.rightIndex > index) {
      ...
      if (this.rightIndex - 2 > index) {
        ...
        else self.goToPage(index, instant);
      } else {
        this.prevPage();
        setTimeout(function () {
          if (typeof instant != "undefined" && instant) {
  ...
  FLIPBOOK.BookWebGL.prototype.nextPageInstant = function (load) {
    ...
    for (i = 0; i < this.pages.length; i++) {
      if (this.pages[i].flippingRight) return;
    }
    ...
    if (this.view == 1) {
      ...
      if (this.flippedright == 1) this.focusLeft(0);
      else this.focusBoth(0);
    }
  ...
  ((FLIPBOOK.BookWebGL.prototype.setRightIndex = function (value) {
    ...
    (FLIPBOOK.BookWebGL.prototype.prevPageInstant = function (load) {
      ...
      var i;
      for (i = 0; i < this.pages.length; i++) {
        if (this.pages[i].flippingLeft) return;
      }
      ...
      if (this.view == 1) {
        ...
        if (this.flippedleft == 1) this.focusRight(0);
        else this.focusBoth(0);
      }
  ...
  FLIPBOOK.BookWebGL.prototype.nextPage = function (load) {
    ...
    if (
      this.flippedright == 1 &&
      this.pages.length * 2 > this.options.numPages &&
    ...
    if (nextPage && flipping < 5) nextPage.showMat();
    ...
    if (this.view == 1) {
      ...
      if (this.flippedright == 1) this.focusLeft(500);
      else this.focusBoth(500);
    }
  ...
  FLIPBOOK.BookWebGL.prototype.updateBookPosition = function () {
    if (this.view == 1) {
      this.isFocusedLeft() && this.rightIndex > 0
        ? this.focusLeft()
        : this.focusRight();
    } else {
      if (this.rightIndex == 0) this.focusRight();
      else if (this.rightIndex >= this.options.numPages) this.focusLeft();
      else this.focusBoth();
    }
  ...
  FLIPBOOK.BookWebGL.prototype.turnPageComplete = function () {
    ...
    if (this.options.cornerCurl) {
      if (this.flippedleft == 0) this.pages[0].startCornerCurl();
      else this.pages[0].stopCornerCurl();
    }
    ...
    for (var i = 0; i < pages.length; i++) {
      var p = pages[i];
      if (p === rightPage || p === leftPage) p.showMat();
  ...
  FLIPBOOK.BookWebGL.prototype.focusLeft = function (time, delay) {
    ...
    var newX = this.options.pageWidth * 0.5 * this.centerContainer.scale.x;
  ...
  FLIPBOOK.BookWebGL.prototype.moveToPos = function (pos, time, delay) {
    if (
      ...
      var self = this;
      this.movingTo = pos;
      var tween = new FLIPBOOK.TWEEN.Tween(this.centerContainer.position)
        .to(
          {
            x: pos,
          },
        ...
        .delay(delay || 0)
        .start();
    } else this.centerContainer.position.x = pos;
  };
  ...
  FLIPBOOK.BookWebGL.prototype.prevPage = function (load) {
    ...
    if (prevPage && flipping < 5) prevPage.showMat();
    ...
    if (this.view == 1) {
      ...
      if (this.flippedleft == 1) this.focusRight(500);
      else this.focusBoth(500);
    }
  ...
  FLIPBOOK.BookWebGL.prototype.updateHtmlLayer = function () {
    ...
    for (var i = 0; i < this.pages.length; i++) {
      if (this.pages[i].flipping) return;
    }
    //flip finnished
    ...
    if (this.options.doublePage) {
      ...
      if (this.rightIndex == 0) {
        ...
        var html =
          this.options.pages[L].htmlContent ||
          this.options.pages[R].htmlContent;
      ...
      if (this.rightIndex == 0) {
        ...
        if (this.options.pages[L].htmlContent) {
          this.$pageLInner.empty();
          var content = jQuery(this.options.pages[L].htmlContent).appendTo(
            this.$pageLInner,
          );
          this.$pageL.show();
          this.htmlLayerVisible = true;
      ...
      if (this.rightIndex == this.pages.length * 2) {
        ...
        if (this.options.pages[R].htmlContent) {
          this.$pageRInner.empty();
          var content = jQuery(this.options.pages[R].htmlContent).appendTo(
            this.$pageRInner,
          );
          this.$pageR.show();
          this.htmlLayerVisible = true;
  ...
  FLIPBOOK.BookWebGL.prototype.zoomTo = function (amount, time, x, y) {
    ...
    if (amount <= 1 && amount <= this.zoom) this.updateBookPosition();
  ...
  FLIPBOOK.BookWebGL.prototype._touchmove = function (e) {
    ...
    if (e.touches.length > 1) {
      ...
      var newZoom =
        this.zoom * Math.pow(this.touchesDist / this.touchesDistStart, 0.2);
      if (newZoom >= this.options.zoomMin && newZoom <= this.options.zoomMax)
        this.zoomTo(
          newZoom,
          20,
          this.touches[0].pageX / 2 + this.touches[1].pageX / 2,
          this.touches[0].pageY / 2 + this.touches[1].pageY / 2,
        );
      this.updateCameraPosition();
      return;
    }
  ...
  FLIPBOOK.BookWebGL.prototype._move = function (e) {
    ...
    var point = e,
      deltaX = (point.pageX - this.pointX) * 0.5,
      deltaY = (point.pageY - this.pointY) * 0.5;
    ...
    if (intersects.length > 0) {
      if (this.onMouseMove == "") {
        if (this.zoom > 1) this.onMouseMove = "scroll";
      }
    } else {
    ...
    if (this.onMouseMove == "scroll") {
      if (this.options.main.tool == "toolSelect") return;
      if (deltaX != 0 || deltaY != 0) {
        this.moved = true;

        this.centerContainer.position.x +=
          (10000 * deltaX) / (this.cameraZ * this.zoom * this.zoom);
        this.centerContainer.position.y -=
          (10000 * deltaY) / (this.cameraZ * this.zoom * this.zoom);
      }
...

{
  ...
  (function (a) {
    ...
    a.Matrix.prototype.transformPoint = function (c) {
      return new a.Point(
        this.m11 * c.x + this.m12 * c.y,
        this.m21 * c.x + this.m22 * c.y,
      );
    };
  })(MOD3);
  (function (a) {
    ...
    a.Vector3.prototype.normalize = function () {
      var a = this.x,
        ...
        a = a * a + b * b + d * d;
      a > 0 &&
        ((a = 1 / Math.sqrt(a)), (this.x *= a), (this.y *= a), (this.z *= a));
    };
  ...
  (function (a) {
    a.Matrix4 = function (a, b, d, e, f, g, h, i, n, m, o, k, p, l, j, q) {
      ...
      if (typeof i != "undefined") this.n24 = i;
  ...
  (function (a) {
    ...
    a.MeshProxy.prototype.analyzeGeometry = function () {
      ...
      j = j(f, j(g, depth));
      if (d == f && j == g)
        ((this.minAxis = a.ModConstant.Y),
          (this.midAxis = a.ModConstant.Z),
          (this.maxAxis = a.ModConstant.X));
      else if (d == f && j == depth)
        ((this.minAxis = a.ModConstant.Z),
          (this.midAxis = a.ModConstant.Y),
          (this.maxAxis = a.ModConstant.X));
      else if (d == g && j == f)
        ((this.minAxis = a.ModConstant.X),
          (this.midAxis = a.ModConstant.Z),
          (this.maxAxis = a.ModConstant.Y));
      else if (d == g && j == depth)
        ((this.minAxis = a.ModConstant.Z),
          (this.midAxis = a.ModConstant.X),
          (this.maxAxis = a.ModConstant.Y));
      else if (d == depth && j == f)
        ((this.minAxis = a.ModConstant.X),
          (this.midAxis = a.ModConstant.Y),
          (this.maxAxis = a.ModConstant.Z));
      else if (d == depth && j == g)
        ((this.minAxis = a.ModConstant.Y),
          (this.midAxis = a.ModConstant.X),
          (this.maxAxis = a.ModConstant.Z));
  ...
  (function (a) {
    ...
    a.Pivot.prototype.apply = function () {
      for (
        var a = this.mod.getVertices(), b = a.length, d = this.pivot, e, f;
        --b >= 0;
      )
  ...
  (function (a) {
    ...
    a.Twist.prototype.apply = function () {
      ...
      for (
        ...
        ((f = b[d]),
          (h = f.getX() * e.x + f.getY() * e.y + f.getZ() * e.z + g),
          this.twistPoint(f, h * c));
    };
    a.Twist.prototype.twistPoint = function (c, b) {
      var d = new a.Matrix4().translationMatrix(c.getX(), c.getY(), c.getZ()),
        d = new a.Matrix4().multiply(
          new a.Matrix4().rotationMatrix(
            this.vector.x,
            this.vector.y,
            this.vector.z,
            b,
  ...
  (function (a) {
    ...
    a.Break.prototype.apply = function () {
      var c = this.mod,
        ...
        h,
        i,
        c = new a.Vector3(0, 0, -(c.minZ + c.depth * this.offset));
      h = c.negate();
  ...
  (function (a) {
    ...
    a.MeshCopperlicht.prototype.setMesh = function (c) {
      a.MeshProxy.prototype.setMesh.call(this, c);
      for (
        var c = this.mesh.getMesh().GetMeshBuffers(), b = [], d = 0;
        d < c.length;
        d++
      )
